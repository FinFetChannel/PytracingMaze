        ...
        if pause:
            ...
            (enx, eny, mplayer, et, shoot, sx, sy, sdir, shoot2,
             sx2, sy2, sdir2, seenx, seeny, lock, enhealth) = agents(enx, eny, maph, posx, posy, rot, et, shoot,
                                                                     ...
                                                                     seenx, seeny, lock, size, enhealth)
##            print( ' agents ok ')
...
##            print(' render ok')
    ...
    if health > 0 and (int(posx) != exitx or int(posy) != exity):
        animate(width, height, mod, move, posx, posy, .5, rot, rot_v, mr, mg, mb, size/2 + 1500, size/2 + 1000, 1000,
...
@njit(cache=True)
def shadow_ray(x, y, z, modr, shot, mapv, refx, refy, maps, lx, ly, lz, enx, eny, sx, sy, sx2, sy2, posx, posy, posz, size, c3, maph):
    ...
    for j in range(height): #vertical loop 
        ...
        for i in range(width): #horizontal vision loop
            if garbage and idx%2 == 1:
                ...
                if (checker == 0 or garbage or
                    (checker == 2 and ((inv and i%2 == j%2) or (not(inv) and i%2 != j%2))) or
                    (checker > 2 and ((rad < height/2.9 and ((inv and i%2 == j%2) or (not(inv) and i%2 != j%2))) or
                    ...
                    sin, cos, sinz = coszo*np.sin(rot_i)*si, coszo*np.cos(rot_i)*si, sinzo*si
                    ...
                    if not shot and z < 1: # shadows
                        modr, c3 = shadow_ray(x, y, z, modr, shot, mapv, refx, refy, maps, lx, ly, lz, enx, eny, sx, sy, sx2, sy2, posx, posy, posz, size, c3, maph)
...
def agents(enx, eny, maph, posx, posy, rot, et, shoot, sx, sy, sdir, shoot2, sx2, sy2, sdir2, mplayer, seenx, seeny, lock, size, enhealth):
    ...
    return(enx, eny, mplayer, et, shoot, sx, sy, sdir, shoot2, sx2, sy2, sdir2, seenx, seeny, lock, enhealth)
...
def drawing(rr, gg, bb, height, width, pause, endmsg, won, health, enhealth, minimap, score, nosplash=True):
    ...
    pg.draw.rect(surfbg, (200-int(health*10), 50+int(health*10), 0),(1205,int(700-62*health),30,int(63*health)))
    pg.draw.rect(surfbg, (50+enhealth*10, 200-enhealth*10, 0),(1245,700-62*enhealth,30,63*enhealth))
...
if __name__ == '__main__':
    pg.init()
    pg.display.set_caption("Welcome to Pytracing maze! Stutters may occur on first run when Numba is compiling")
