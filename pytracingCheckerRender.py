...
def main():
    ...
    timer, autores, checker = 0, 1, 1
    width, height, mod, inc, rr, gg, bb, count = adjust_resol(300)
    ...
    endmsg = " Game paused. Numba may need more compiling "
    ...
    new_game(width, height, mod, inc, posx, posy, .99, rot, rot_v, mr, mg, mb, size/2, size/2, 1000, maph, exitx, exity, mapr, mapt, maps,
            ...
            if event.type == pg.KEYDOWN:
                ...
                if event.key == ord('q'): # change resolution or fps
                    if autores:
                        ...
                        if width > 100 :
                            width, height, mod, inc, rr, gg, bb, count = adjust_resol(int(width*0.8))
                if event.key == ord('e'): # change resolution or fps
                    if autores:
                        fps_lock = min(120, fps_lock + 10)
                    else:
                        width, height, mod, inc, rr, gg, bb, count = adjust_resol(int(width*1.1))
...
##            print(' render ok')
            ...
            if (int(posx) == exitx and int(posy) == exity):
                ...
                animate(width, height, mod, inc, posx, posy, .5, rot, rot_v, mr, mg, mb, lx, ly, lz,
                        mplayer, exitx, exity, mapr, mapt, maps, rr, gg, bb, enx, eny, sx, sy, sx2, sy2,
            ...
            if enx == 0:
                if not respawn:
                    ...
                    pg.mixer.Channel(3).play(killfx)
                    run = 1
                    respawn = 1
                ...
                if shoot2 or sstart2 != None:
                    ...
                    if (sx2 - posx)**2 + (sy2 - posy)**2 < 0.01:
                        ...
                        if health <= 0:
                            ...
                            enx, eny, seenx, seeny, lock, health, won  = 0, 0, 0, 0, 0, 5, 0
            ...
            posx, posy, rot, rot_v, shoot = movement(pg.key.get_pressed(),posx, posy, rot, rot_v, maph, et, shoot, sstart)
...
def new_game(width, height, mod, inc, posx, posy, posz, rot, rot_v, mr, mg, mb, lx, ly, lz, maph, exitx, exity, mapr, mapt, maps,
             rr, gg, bb, enx, eny, sx, sy, sx2, sy2, size, checker, count, fb, fg, fr, pause, endmsg, won, health, minimap, score, ani):

    animate(width, height, mod, inc, posx, posy, posz, rot, rot_v, mr, mg, mb, lx, ly, lz, maph, exitx, exity, mapr, mapt, maps,
             rr, gg, bb, enx, eny, sx, sy, sx2, sy2, size, checker, count, fb, fg, fr, pause, endmsg, won, health, minimap, score, ani)
#enx, eny, seenx, seeny, lock, run, shoot, sx, sy, sstart, et, count, health, sdir, sdir2, shoot2, sx2, sy2, sstart2, won, et, run, respawn
...
##    posx, posy = posx + inc*(1-inverse)/.5, posy + inc*(1-inverse)/.5
    garbage = not(count)
    idx = 0
        ...
        for i in range(width): #horizontal vision loop
            if (not(checker) or i == 0 or i == width -1 or j == 0 or j == height -1 or (inverse and i%2 == j%2) or (not(inverse) and i%2 != j%2)):
                ...
                if z > 1: # ceiling
                    ...
                    z = 0
                    xx = int(x%1*100) + int(y%1*100)*100
                    if int(x) == exitx and int(y) == exity: #exit
                        c1, c2, c3 = fb[xx]/655, fb[xx]/555, fb[xx]/255
                    else:
                        c1, c2, c3 = fg[xx]/255, fg[xx]/255, fb[xx]/255
                    ...
                    if mapt[int(x)][int(y)]: # textured walls
                        ...
                        if maps[int(x)][int(y)]:
                            c1, c2, c3 = c1*fr[xx]/455, c2*fg[xx]/455, c3*fb[xx]/755
                        else:
                            xx = fr[xx]/255
                ...
                if not shot and z < 1:
                    ...
                    elif sx2 != -1 and maph[int(sx2)][int(sy2)] > 1:
                        shot, c3, limodr = 1, c3 * 0.9, 0.6
        ...
        for j in range(height): #vertical loop        
            for i in range(width): #horizontal vision loop
                if (i > 0 and i < width -1 and j > 0 and j < height -1 and ((inverse and i%2 != j%2) or (not(inverse) and i%2 == j%2))):
                    if abs(pr[idx-1] - pr[idx+1]) < 0.1 and abs(pg[idx-1] - pg[idx+1]) < 0.1 and abs(pb[idx-1] - pb[idx+1]) < 0.1 :
                        pr[idx] = (pr[idx-1] + pr[idx+1])/2
                        pg[idx] = (pg[idx-1] + pg[idx+1])/2
                        pb[idx] = (pb[idx-1] + pb[idx+1])/2
                    elif abs(pr[idx-width] - pr[idx+width]) < 0.1 and abs(pg[idx-width] - pg[idx+width]) < 0.1 and abs(pb[idx-width] - pb[idx+width]) < 0.1 :
...
def adjust_resol(width):
    ...
    return width, height, mod, inc, rr, gg, bb, 0
...
def drawing(rr, gg, bb, height, width, pause, endmsg, won, health, minimap, score, nosplash=True):
    ...
    if pause:        
        screen.blit(font2.render(" FinFET's PyTracing Maze  ", 0, pg.Color("red")),(45,45))
        screen.blit(font2.render(" FinFET's PyTracing Maze  ", 0, pg.Color("blue")),(55,55))
        screen.blit(font2.render(" FinFET's PyTracing Maze ", 0, pg.Color("white")),(50,50))
...
def animate(width, height, mod, inc, posx, posy, posz, rot, rot_v, mr, mg, mb, lx, ly, lz,
            maph, exitx, exity, mapr, mapt, maps, pr, pg, pb, enx, eny, sx, sy, sx2, sy2,
            size, checker, count, fb, fg, fr, pause, endmsg, won, health, minimap, score, ani):
    for i in range(60):
        rr, gg, bb = super_fast(width, height, mod, inc, posx, posy, posz+ani*i, rot, rot_v, mr, mg, mb, lx, ly, lz,
                                maph, exitx, exity, mapr, mapt, maps, pr, pg, pb, enx, eny, sx, sy, sx2, sy2,
                                size, checker, count, fb, fg, fr)
        count += 1
